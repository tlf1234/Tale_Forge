...
export default function WorksPage() {
  ...
  const fetchIPFSImage = async (cid: string) => {
    try {
      ...
      const response = await fetch(
        `https://blue-casual-wombat-745.mypinata.cloud/ipfs/${cid}`
      );
  ...
  const loadWorks = async () => {
    ...
    try {
      ...
      console.log(
        "【作品管理】请求API: /api/authors/${address}/stories，作者地址:",
        address
      );
      ...
      // 获取所有 IPFS 图片
      const ipfsStories = fetchedStories.filter((story: Story) =>
      ...
      const imagePromises = ipfsStories.map(async (story: Story) => {
        console.log(
          `[IPFS] 开始处理故事 ${story.id} 的封面: ${story.coverCid}`
        );
      ...
      console.log(
        "[IPFS] 所有图片加载完成，当前图片缓存:",
        Object.keys(loadedImages)
      );
      ...
      const formattedWorks: Work[] = fetchedStories.map((story: Story) => ({
        ...
        coverImage: story.coverCid?.startsWith("data:image")
          ? story.coverCid
          : story.coverCid?.startsWith("Qm")
          ? loadedImages[story.coverCid] ||
            `https://blue-casual-wombat-745.mypinata.cloud/ipfs/${story.coverCid}`
