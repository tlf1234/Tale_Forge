...
// 优化后的简洁美观提示弹窗
const SimpleDialog = ({ message, onClose }: { message: string; onClose: () => void }) => (
  <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/40 backdrop-blur-sm animate-fadeIn">
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full mx-4 overflow-hidden transform transition-all animate-scaleIn">
      <div className="p-6 sm:p-8">
        ...
        <div className="text-center mb-6">
          <p className="text-gray-800 dark:text-gray-100 text-lg font-medium leading-relaxed tracking-normal">
            {message}
          </p>
        </div>
        ...
        <div className="flex justify-center">
          <button
            onClick={onClose}
            className="px-6 py-2.5 bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white rounded-lg font-medium transition-colors duration-200 shadow-md hover:shadow-lg"
          >
            <span className="tracking-wide">确定</span>
          </button>
        </div>
      </div>
    </div>
...
const ChapterListDialog = ({ 
  ...
  onEditClick: (e: React.MouseEvent, chapterId: string, title: string) => void;
  onDeleteClick: (e: React.MouseEvent, chapterId: string) => void;
  onPublishClick: (chapterId: string) => void;
  ...
  const [displayedChapters, setDisplayedChapters] = useState<ChapterType[]>([]);
  ...
  // 初始化显示的章节
  useEffect(() => {
    const initialChapters = sortedChapters;
    setDisplayedChapters(initialChapters);
  }, [sortedChapters]);
  ...
  const handleScroll = useCallback(() => {
    ...
    const now = Date.now();
    if (now - lastScrollTime.current < 200) return; // 200ms 节流
  ...
  // 当chapters更新时，追加到displayedChapters
  useEffect(() => {
    setDisplayedChapters(prev => {
      ...
      // 合并并按order倒序排序
      return [...prev, ...newChapters].sort((a, b) => b.order - a.order);
    });
  }, [chapters]);
  ...
  // 滚动到选中的章节
  useEffect(() => {
    if (currentChapterId && chapterRefs.current[currentChapterId]) {
      const chapterElement = chapterRefs.current[currentChapterId];
      const container = scrollContainerRef.current;
      if (chapterElement && container) {
        const containerRect = container.getBoundingClientRect();
        const chapterRect = chapterElement.getBoundingClientRect();
        
        // 计算需要滚动的距离，使选中的章节位于容器中间
        const scrollTo = chapterElement.offsetTop - (containerRect.height - chapterRect.height) / 2;
        container.scrollTo({
          top: scrollTo,
          behavior: 'smooth'
        });
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div className="flex items-center gap-3">
              <h3 className="text-xl font-semibold text-gray-800">章节列表</h3>
              <div className="flex items-center gap-2 text-sm text-gray-500">
                <span className="px-2 py-1 bg-gray-50 rounded-md">总章节: {totalChapters}</span>
                <span className="px-2 py-1 bg-green-50 text-green-700 rounded-md">已发布: {totalPublished}</span>
                <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded-md">草稿: {totalDrafts}</span>
              </div>
            </div>
          ...
          <div className="px-6 py-4 border-b border-gray-100">
            <div className="relative">
              <input
                ...
                placeholder="搜索章节..."
                className="w-full px-4 py-2.5 pr-12 bg-gray-50 border-0 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white transition-all text-sm"
              />
              {searchKeyword && (
                <button
                  onClick={onClearSearch}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-gray-600 hover:text-red-500 bg-gray-200 hover:bg-gray-300 rounded-full transition-all flex items-center justify-center"
                  title="清除搜索"
                >
          ...
          >
            <div className="grid grid-cols-4 gap-4">
              {displayedChapters.map((chapter) => (
                <div
                  ...
                  className={`group flex flex-col p-4 rounded-xl border border-gray-100 hover:border-blue-100 cursor-pointer transition-all ${
                    currentChapterId === chapter.id 
                      ? 'bg-blue-50/50 border-blue-200 shadow-sm' 
                      : 'bg-white hover:bg-blue-50/30 hover:shadow-sm'
                  }`}
                  onClick={() => onChapterSelect(chapter.id)}
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-400">#{chapter.order}</span>
                      ...
                      {chapter.status !== 'published' && (
                        <>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              onEditClick(e, chapter.id, chapter.title);
                            }}
                  ...
                  <div className="flex-1">
                    <h4 className="text-sm font-medium text-gray-700 line-clamp-2 mb-2 min-h-[2.5rem]">
                      {chapter.title}
                    </h4>
                    <div className="flex items-center text-xs text-gray-500">
                      <span className="flex items-center">
                        <FaBook className="w-3 h-3 mr-1 text-gray-400" />
                        {chapter.wordCount} 字
                      </span>
                      {chapter.updatedAt && (
            ...
            {/* 全部加载完成提示 */}
            {!hasMore && displayedChapters.length > 0 && (
...
const [showSuccessDialog, setShowSuccessDialog] = useState(false);
  ...
  // 添加新的状态
  const [showFullChapterList, setShowFullChapterList] = useState(false);

  // 添加弹窗专用的章节列表状态
  const [dialogChapters, setDialogChapters] = useState<ChapterType[]>([]);
  ...
  // 修改状态定义部分，添加加载的章节范围跟踪和章节统计
  const [dialogLoadedRange, setDialogLoadedRange] = useState<{min: number, max: number} | null>(null);
  const [dialogTotalChapters, setDialogTotalChapters] = useState(0);
  const [dialogTotalPublished, setDialogTotalPublished] = useState(0);
  const [dialogTotalDrafts, setDialogTotalDrafts] = useState(0);

  // 显示弹窗的函数
  const showSimpleDialog = (message: string) => {
  ...
  // 添加弹窗章节列表专用的加载方法
  const loadDialogChapters = async (page: number = 1) => {
    try {
      if (!currentStory?.id) {
        throw new Error('未选择故事');
      }

      // 获取章节统计信息
      const statsResponse = await fetch(`/api/stories/${currentStory.id}/chapters/stats`);
      if (!statsResponse.ok) {
      ...
      if (page === 1) {
        ...
        start = Math.max(1, end - DIALOG_PAGE_SIZE + 1);
        
        // 记录已加载范围
        setDialogLoadedRange({ min: start, max: end });
      } else {
        // 后续页面继续加载前面的章节
        if (!dialogLoadedRange) {
          return { chapters: [], hasMore: false };
        }
        
        // 从当前已加载最小序号的前一章开始加载
        end = dialogLoadedRange.min - 1;
        start = Math.max(1, end - DIALOG_PAGE_SIZE + 1);
        
        // 更新已加载范围
        if (end >= start) {
          setDialogLoadedRange({ 
            min: Math.min(dialogLoadedRange.min, start),
            max: dialogLoadedRange.max
          });
        }
      }
      ...
      const response = await fetch(
        `/api/stories/${currentStory.id}/chapters/range?start=${end}&end=${start}`
      );
      ...
      const processedChapters = chaptersData.map((chapter: any) => ({
        ...
        status: (chapter.status?.toLowerCase() || 'draft') as ChapterType['status'],
        price: chapter.price,
  ...
  const loadMoreDialogChapters = async () => {
    if (!dialogHasMore) return;
    
    const nextPage = dialogCurrentPage + 1;
    const result = await loadDialogChapters(nextPage);
    
    if (result.chapters.length > 0) {
      setDialogChapters(prev => [...prev, ...result.chapters]);
      setDialogCurrentPage(nextPage);
      setDialogHasMore(result.hasMore);
    }
  };
  // 修改弹窗显示处理函数
  const handleShowFullChapterList = async () => {
    try {
      if (!currentStory?.id) {
        showSimpleDialog('请先选择作品');
        return;
      }
      ...
      // 加载第一页数据
      const result = await loadDialogChapters(1);

      if (result.chapters.length > 0) {
        setDialogChapters(result.chapters);
        setDialogHasMore(result.hasMore);
        setDialogTotalChapters(stats.total || 0);
        setDialogTotalPublished(stats.published || 0);
        setDialogTotalDrafts((stats.total || 0) - (stats.published || 0));
        setShowFullChapterList(true);
      } else {
        showSimpleDialog('暂无章节');
      }
    } catch (error) {
      console.error('显示章节列表失败:', error);
      showError(error, '显示章节列表失败');
    }
  };

  //关闭弹窗
  const handleCloseDialog = () => {
    setShowFullChapterList(false);
    setDialogChapters([]);
    setDialogCurrentPage(1);
    setDialogHasMore(true);
  };
  ...
  */
  ...
  const handleAddChapter = async (volumeId?: string) => {
    try {
      ...
      // 直接从已加载的章节列表中统计草稿数量
      // 注意：使用小写的 'draft' 而不是大写的 'DRAFT'
      console.log('handleAddChapter 章节列表:', chapters);
      const draftCount =  chapters.filter(chapter => (chapter.status || '').toLowerCase()
       === 'draft'
      ).length;
      console.log('handleAddChapter 草稿数量:', draftCount);
      // 如果草稿数量已达到5章，显示提示并返回
      if (draftCount >= 5) {
        showSimpleDialog('最多缓存5章未发布章节，请先发布章节后再创建新章节');
        return;
      }

      // 如果草稿数量未达到限制，直接继续创建流程
      proceedWithAddChapter();

    } catch (error) {
      showError(error, '创建章节失败');
    }
  };

  // 继续创建章节的辅助函数，使用您原有的命名规范
  const proceedWithAddChapter = () => {
    // 获取当前最大章节序号
    const maxOrder = chapters.reduce((max, chapter) => 
      Math.max(max, chapter.order), 0);
    console.log('当前最大章节序号:', maxOrder);
    // 设置新章节的序号和标题
    const newOrder = maxOrder + 1;
    
    // 根据当前设置新章节标题
    const newTitle = `第${newOrder}章  `;
    setNewChapterTitle(newTitle);
  ...
  const handleConfirmAddChapter = async () => {
    try {
      ...
      const chapterNumber = totalChapters + 1;
      const prefix = `第${chapterNumber}章  `;
      
      // 如果标题只有前缀，提示用户添加标题
      if (newChapterTitle.trim() === prefix.trim()) {
        toast.error('请输入章节标题');
        return;
      }
      ...
      // 获取最新章节的序号
      const orders = recentChapters.map(chapter => chapter.order);
      const maxOrder = orders.length > 0 ? Math.max(...orders) : 0;
      const nextOrder = maxOrder + 1;
      
      console.log('新章节序号计算:', {
        existingOrders: orders,
        maxCurrentOrder: maxOrder,
  ...
  const loadChapterList = async () => {
    try {
      ...
      const processedChapters = chapters.map(chapter => ({
        ...chapter,
        title: chapter.title || `第${chapter.order}章`,  // 确保有标题
  ...
  const handleSearchChapters = async (keyword: string) => {
    ...
    try {
      ...
      // 处理搜索结果，确保每个章节都有完整的数据结构
      const processedResults = searchResults.map(chapter => {
        // 确保含有所有必要的字段
        return {
          id: chapter.id,
          title: chapter.title || `第${chapter.order}章`,
      ...
      // 按照章节序号排序
      const sortedResults = processedResults.sort((a, b) => b.order - a.order);
      
      // // 更新搜索结果状态
      setFilteredChapters(sortedResults);
      
      // // 更新对话框中显示的章节
      setDialogChapters(sortedResults);
      setDialogHasMore(false); // 搜索模式下不加载更多
    } catch (error) {
  ...
  const loadChapter = async (chapterId: string) => {
    try {
      ...
      // 检查章节状态，如果是已发布状态，则启用预览模式
      console.log('【loadChapter】章节状态:', chapter.status);
      if (chapter.status?.toLowerCase() === 'published') {
        console.log('【loadChapter】发现已发布章节，启用预览模式');
        setIsPreview(true);
      } else {
        console.log('【loadChapter】未发布章节，不启用预览模式');
        setIsPreview(false);
      }
  ...
  const handlePublishChapter = async (chapterId: string) => {
    ...
    try {
      ...
      // 4. 检查当前章节是否是第一个章节或前一个章节已发布
      const isFirstChapter = currentChapterIndex === 0;
      const isPreviousChapterPublished = previousChapter && previousChapter.status?.toLowerCase() === 'published';
      
      if (!isFirstChapter && !isPreviousChapterPublished) {
        // 如果不是第一个章节且前一个章节未发布，显示错误提示
        const prevChapterTitle = previousChapter?.title || `第${previousChapter?.order || '?'}章`;
        toast.error(`请按顺序发布章节！您需要先发布 ${prevChapterTitle}`, {
          style: {
            background: '#fff',
            color: '#333',
            padding: '16px',
      ...
      console.error('【handlePublishChapter】检查章节顺序失败:', error);
      showError(error, '章节发布准备失败');
    }
  };
  ...
  const handleConfirmPublish = async () => {
    ...
    try {
      ...
      // 先保存最新内容 - 20%进度
      setCreateProgress(20);
      console.log('【handleConfirmPublish】准备保存最新内容');
      try {
        await handleSaveClick({ stopPropagation: () => {} } as React.MouseEvent, chapterToPublish);
        console.log('【handleConfirmPublish】已保存最新内容');
      } catch (saveError) {
        console.error('【handleConfirmPublish】保存内容失败:', saveError);
        throw new Error('保存章节内容失败，请稍后重试');
      }

      // 第一步：准备发布章节 - 30%进度
      setCreateProgress(30);
      setCreateStatus(CreateStatus.UPLOADING);
      console.log('【handleConfirmPublish】准备发布章节');
      const prepareResponse = await fetch(`/api/stories/${storyId}/chapters/${chapterToPublish}/prepare-publish`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (!prepareResponse.ok) {
        ...
        console.error('【handleConfirmPublish】准备发布失败:', errorData);
        throw new Error(errorData.error || '准备发布失败');
      }

      const prepareResult = await prepareResponse.json();
      console.log('【handleConfirmPublish】准备发布成功，结果:', prepareResult);
      ...
      setCreateStatus(CreateStatus.SAVING_DATABASE);
      setCreateProgress(80);
      console.log('【handleConfirmPublish】完成章节发布, 参数:', {
        chapterToPublish,
        txHash: receipt.transactionHash
      });

      const completeResponse = await fetch(`/api/stories/${storyId}/chapters/${chapterToPublish}/complete-publish`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (!completeResponse.ok) {
        ...
        console.error('【handleConfirmPublish】完成发布失败:', errorData);
        throw new Error(errorData.error || '完成发布失败');
      }

      const publishedChapter = await completeResponse.json();
      console.log('【handleConfirmPublish】发布完成，返回数据:', publishedChapter);
      ...
      for (const img of permanentImages) {
        ...
        processedContent = processedContent.replace(
          ...
          (match) => {
            ...
            // 安全地获取属性
            const alt = originalImg?.getAttribute('alt') || '';
            const title = originalImg?.getAttribute('title') || '';
            const width = originalImg?.getAttribute('width') || '';
            const height = originalImg?.getAttribute('height') || '';
            
            // 创建新的img标签，保持与原代码相同的变量
            return `<img class="chapter-image editor-image" src="${imagePath}" data-position="${img.position}" data-filename="${fileName}" alt="${alt}" title="${title}" width="${width}" height="${height}" draggable="true">`;
          }
        );
      }
  ...
  */
  ...
  const handleContentChange = useCallback((newContent: string) => {
    ...
    // 如果当前章节已发布，不允许修改
    if (currentChapter?.status?.toLowerCase() === 'published') {
      console.log('【write/page】当前章节已发布，不允许修改内容');
      return;
    }
  ...
  <div className={styles.toolbar}>
    ...
    <div className={styles.toolbarRight}>
      <button                                                                                                                                                                                                                                                                                     className={`${styles.iconButton} ${isPreview ? styles.active : ''}`}
        onClick={() => {
          // 如果章节已发布则不允许退出预览模式
          if (currentChapter?.status?.toLowerCase() === 'published' && isPreview) {
            return; // 已发布章节不能退出预览模式
          }
          setIsPreview(!isPreview);
        }}
        disabled={currentChapter?.status?.toLowerCase() === 'published' && isPreview}
        title={currentChapter?.status?.toLowerCase() === 'published' ? "已发布章节只能预览" : "预览"}
      >
                ...
                {!showSettings && (
                  ...
                  <div className={styles.storySection}>
                    <div className={styles.storySelectorHeader}>
                      ...
                      <IconButton
                         icon={<FaList />}
                        onClick={handleShowFullChapterList}
                        title="查看章节列表"
                      />
                    </div>
                ...
                <div className={styles.editorContent}>
                  <div className={styles.editorInner}>
                    <Editor
                      ...
                      editable={!isPreview && currentChapter?.status?.toLowerCase() !== 'published'}
                      className={isPreview ? styles.previewContent : styles.content}
                      placeholder={currentChapter?.status?.toLowerCase() === 'published' ? '该章节已发布，无法编辑' : '开始创作你的故事...'}
                      onSave={async () => {
                        if (currentChapter?.status?.toLowerCase() !== 'published') {
                          await handleSave();
                        }
      ...
      {showFullChapterList && (
        <ChapterListDialog
          ...
          onClose={() => {
            ...
            setDialogCurrentPage(1);
            setDialogHasMore(true);
            setDialogTotalChapters(0);
            setDialogTotalPublished(0);
            setDialogTotalDrafts(0);
          }}
          ...
          onPublishClick={handlePublishChapter}
          searchKeyword={chapterSearchKeyword}
          onSearch={handleSearchChapters}
          onClearSearch={clearSearch}
          currentChapterId={currentChapterId || undefined}
          hasMore={dialogHasMore}
          onLoadMore={loadMoreDialogChapters}
          totalChapters={dialogTotalChapters}
          totalPublished={dialogTotalPublished}
          totalDrafts={dialogTotalDrafts}
        />
      )}
    </WalletRequired>
  )
