...
contract StoryManager is Ownable, ReentrancyGuard {
    ...
    struct Story {
        ...
        mapping(uint256 => uint256) dailyFavorites;  // 每日收藏数
        mapping(uint256 => uint256) dailyShares;     // 每日分享数
    ...
    struct InteractionBatch {
        ...
        uint256 storiesCount;         // 处理的作品数量（可选，方便查询）
    }

    // 批量互动数据
    struct BatchInteractions {
        uint256 storyId;              // 作品ID
        uint256 newLikes;             // 新增点赞数
        uint256 newComments;          // 新增评论数
        uint256 newFavorites;         // 新增收藏数
        uint256 newShares;            // 新增分享数
    }

    // 批次记录映射
    mapping(uint256 => InteractionBatch) public batches;//
    uint256[] public batchIds;        // 所有批次ID数组，用于遍历

    // 批量处理事件
    event BatchInteractionsProcessed(
        uint256 batchId,
    ...
    //作品评定因子：更新字数、点赞、评论、收藏、分享、打赏、NFT收益
    //这些因子是批量传入的，不是单个增加，所以需要修改这些函数
    // 批量更新互动数据
    function batchUpdateInteractions(
        uint256 batchId,
        BatchInteractions[] memory interactions
    ) external onlyOwner {
        // 检查批次ID是否已存在
        require(batches[batchId].timestamp == 0, "Batch already processed");
        
        // 记录批次信息
        batches[batchId] = InteractionBatch({
            batchId: batchId,
            timestamp: block.timestamp,
            storiesCount: interactions.length  // 可选字段
        });
        batchIds.push(batchId);
        
        // 处理每个作品的互动数据
        for (uint256 i = 0; i < interactions.length; i++) {
            BatchInteractions memory interaction = interactions[i];
            uint256 storyId = interaction.storyId;
            ...
            if (!story.isCompleted && !story.isAbandoned) {
                ...
                // 更新点赞数据
                if (interaction.newLikes > 0) {
                    story.dailyLikes[today] = story.dailyLikes[today].add(interaction.newLikes);
                    story.likeCount = story.likeCount.add(interaction.newLikes);
                }
                
                // 更新评论数据
                if (interaction.newComments > 0) {
                    story.dailyComments[today] = story.dailyComments[today].add(interaction.newComments);
                    story.commentCount = story.commentCount.add(interaction.newComments);
                }
                
                // 更新收藏数据 (未来可能会用到)
                if (interaction.newFavorites > 0) {
                    // 如果将来添加收藏功能，可以在此处更新
                }
                
                // 更新分享数据 (未来可能会用到)
                if (interaction.newShares > 0) {
                    // 如果将来添加分享功能，可以在此处更新
                }
        ...
        emit BatchInteractionsProcessed(batchId, interactions.length, block.timestamp);
    }
    ...
    function getBatchInfo(uint256 batchId) external view returns (
        ...
        InteractionBatch storage batch = batches[batchId];
        return (batch.batchId, batch.timestamp, batch.storiesCount);
    }

    // 获取最近的批次ID
    function getRecentBatchIds(uint256 count) external view returns (uint256[] memory) {
        uint256 batchCount = batchIds.length;
        uint256 resultCount = count > batchCount ? batchCount : count;
        
        uint256[] memory result = new uint256[](resultCount);
        for (uint256 i = 0; i < resultCount; i++) {
            result[i] = batchIds[batchCount - i - 1];
        }
