...
contract AuthorManager is Ownable, ReentrancyGuard {
    ...
    struct Author {
        ...
        uint256 likeCount; // 总点赞数
        uint256 followerCount; // 粉丝数量
        bool isSpecial; // 是否为特邀作者
        address tokenAddress; // 作者代币地址
    }
    ...
    address public tokenFactory; // 代币工厂合约地址
    ...
    event AuthorTokenSet(address indexed author, address indexed tokenAddress);
    event AuthorLikesUpdated(address indexed author, uint256 newLikeCount);
    event AuthorFollowersUpdated(address indexed author, uint256 newFollowerCount);
    event SpecialAuthorStatusChanged(address indexed author, bool isSpecial);
    ...
    modifier onlyTokenFactory() {
        require(msg.sender == tokenFactory, "Only token factory can call");
        _;
    }
    ...
    // 新增：设置作者代币地址
    function setAuthorToken(address author, address tokenAddress) external onlyTokenFactory {
        require(authors[author].isActive, "Author not registered");
        require(authors[author].tokenAddress == address(0), "Author already has token");
        
        authors[author].tokenAddress = tokenAddress;
        emit AuthorTokenSet(author, tokenAddress);
    }
    
    // 新增：更新作者点赞数
    function updateAuthorLikes(address author, uint256 newLikeCount) external onlyOwner {
        require(authors[author].isActive, "Author not registered");
        
        authors[author].likeCount = newLikeCount;
        emit AuthorLikesUpdated(author, newLikeCount);
    }
    
    // 新增：更新作者粉丝数
    function updateAuthorFollowers(address author, uint256 newFollowerCount) external onlyOwner {
        require(authors[author].isActive, "Author not registered");
        
        authors[author].followerCount = newFollowerCount;
        emit AuthorFollowersUpdated(author, newFollowerCount);
    }
    
    // 新增：设置特邀作者状态
    function setSpecialAuthorStatus(address author, bool isSpecial) external onlyOwner {
        require(authors[author].isActive, "Author not registered");
        
        authors[author].isSpecial = isSpecial;
        emit SpecialAuthorStatusChanged(author, isSpecial);
    }
    
    // 新增：检查作者是否已注册
    function isAuthorRegistered(address author) external view returns (bool) {
        return authors[author].isActive;
    }
    
    // 新增：检查作者是否为特邀作者
    function isSpecialAuthor(address author) external view returns (bool) {
        return authors[author].isSpecial;
    }
    
    // 新增：获取作者统计数据
    function getAuthorStats(address author) external view returns (uint256 totalWordCount, uint256 likeCount, uint256 followerCount) {
        Author storage authorData = authors[author];
        return (authorData.totalWordCount, authorData.likeCount, authorData.followerCount);
    }
